...
import sys
from fresh_comp.utils import load_file, save_file
from fresh_comp.utils import read_train_user_data
...
class UserCF(object):
    ...
    def init_test(self, origin_data):
        ...
        for user, item in origin_data:
            users.add(user)
            tests.setdefault(user, set())
            tests[user].add(item)
        return users, tests

if __name__ == '__main__':
    trainset, testset = read_train_user_data(train_rate=0.8)
    user_cf = UserCF()
    # users, tests = user_cf.init_test(testset)
    # 开始训练
    user_cf.train(trainset)
    r = user_cf.recommend(10001082, 10, 10)
    print(r)
